...
onMounted(() => {
  ...
  guiMaterialFolder.add(material, 'wireframe').name('立方体 线框');
  const guiPositionFolder = gui.addFolder('Position 位置参数');
  guiPositionFolder.add(cube.position, 'x').min(-5).max(5).step(0.1).name('立方体 x 轴');
  guiPositionFolder.add(cube.position, 'y').min(-5).max(5).step(0.1).name('立方体 y 轴');
  guiPositionFolder.add(cube.position, 'z').min(-5).max(5).step(0.1).name('立方体 z 轴');
  onBeforeUnmount(() => {
    gui.destroy();
  });
});
